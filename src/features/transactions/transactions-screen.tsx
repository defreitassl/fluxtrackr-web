...
function canShowMonthlySummary(startDate: string, endDate: string, invalidRange: boolean) {
...
export function TransactionsScreen() {
  ...
  const summaryIsAvailable = canShowMonthlySummary(filters.startDate, filters.endDate, invalidRange);
  ...
  const monthlySummary = useTransactionMonthlySummary(summaryPeriod, { enabled: summaryIsAvailable });
  ...
  const refresh = () => {
    if (!transactions.isFetching && !monthlySummary.isFetching && !invalidRange) {
      ...
      if (summaryIsAvailable) refetches.push(monthlySummary.refetch({ cancelRefetch: false }));
  ...
  return <section className="resource-screen" aria-labelledby="transactions-title">
    ...
    {summaryIsAvailable ? <TransactionPeriodSummary isError={monthlySummary.isError} isLoading={monthlySummary.isFetching} periodLabel={formatSummaryPeriod(summaryPeriod)} summary={monthlySummary.data} /> : <p className="transaction-summary-unavailable" role="status">O resumo mensal está disponível para períodos dentro do mesmo mês.</p>}
